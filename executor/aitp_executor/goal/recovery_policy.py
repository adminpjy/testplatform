...
class RecoveryPolicy:
    strategy_catalog: dict[str, list[dict[str, Any]]] = {
        ...
        "navigation_target_not_verified": [
            _strategy("wait_and_reobserve_target_page", "等待新页面加载后重新验证目标系统"),
            _strategy("check_new_page_url_title", "检查新页面 URL、标题和权限提示"),
            _strategy("check_global_interruption", "检查证书、登录、权限或空白页中断"),
            _strategy("ask_human", "请求人工确认目标系统成功标识", automatic=False),
        ],
        "navigation_popup_not_switched": [
            _strategy("switch_to_new_page", "切换到新打开的页面后继续验证"),
            _strategy("wait_and_reobserve_target_page", "等待新页面加载后重新观察"),
            _strategy("ask_human", "请求人工确认新页面内容", automatic=False),
        ],
        "target_page_load_failed": [
            _strategy("wait_and_reload_target_page", "等待后刷新目标页面"),
            _strategy("check_certificate_or_permission", "检查证书风险页、权限页或登录页"),
            _strategy("ask_human", "请求人工确认目标系统打开方式", automatic=False),
        ],
...
def _detect_from_text(text: str) -> str | None:
    ...
    patterns = [
        ...
        ("navigation_target_not_verified", ["navigation_target_not_verified", "未出现目标系统证据", "新页面未出现目标系统证据"]),
        ("navigation_popup_not_switched", ["navigation_popup_not_switched"]),
        ("target_page_load_failed", ["target_page_load_failed"]),
...
def _summary(failure_type: str, *, target: str | None, error_summary: str | None) -> str:
    labels = {
        ...
        "navigation_target_not_verified": "已打开或切换页面，但未确认进入目标系统。",
        "navigation_popup_not_switched": "目标系统可能打开了新页面，但未完成页面切换。",
        "target_page_load_failed": "目标系统页面打开失败或被中断页阻挡。",
