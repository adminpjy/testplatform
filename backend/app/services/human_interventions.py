...
def enable_rule_draft(db: Session, *, draft_id: int) -> AbilityRule:
    ...
    rule = AbilityRule(
        ...
        intent=str(content.get("intent") or content.get("business_intent") or draft.reason or ""),
        status="active",
        priority=int(content.get("priority") or 60),
        ...
        fallback_strategies_json=(content.get("fallback_strategies") if isinstance(content.get("fallback_strategies"), dict) else {"source": "human_intervention"}),
        risk_level=str(content.get("risk_level") or "medium"),
        confidence_threshold=float(content.get("confidence_threshold") or 0.65),
